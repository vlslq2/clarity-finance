-- Step 1: Update the function to use direct casting for JOINs.
...
BEGIN
    ...
    FROM
        budgets b
    LEFT JOIN
        categories c ON b.category_id::text = c.id::text -- Cast to TEXT for join
    LEFT JOIN
        monthly_expenses me ON b.category_id::text = me.category_id::text -- Cast to TEXT for join
    WHERE
        b.user_id = p_user_id;
END;
$$;

-- Step 2: Drop the unused category_uuid column from the budgets table.
ALTER TABLE public.budgets DROP COLUMN IF EXISTS category_uuid;